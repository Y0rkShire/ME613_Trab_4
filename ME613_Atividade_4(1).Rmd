...
p_values_table <- data.frame(P_Valor = interaction_p_values)
p_values_table$P_Valor <- round(p_values_table$P_Valor, digits = 4)
p_values_table$Estimado <- round(interaction_estimates,digits = 4)

hist(interaction_p_values, main = "Distribuição dos p-valores das interações",ylab = "frequência",xlab = "p-valores")
kable(p_values_table,format = "simple")%>%
  kable_styling(full_width = FALSE)
```

Surpreendentemente muitas variáveis tem p-valores menores do que o 0.05 estipulado como limiar de significância, isso indica que dada uma análise mais meticulosa da relação interativa entre as variáveis, podemos considerar em adicionar variáveis interativas ao modelo que ajudem a explicar melhor alguns desvios dos resíduos,como por exemplo nas interações \( X_{marca} X_{idade} \) onde a maioria teve p-valor menor que 0.05,valores estimados significativos e não conteve valores faltates como a variável \( X_{combustivel} X_{marca} \) tiveram.
